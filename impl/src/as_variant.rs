...
pub fn expand(input: &DeriveInput, trait_name: &'static str) -> Result<TokenStream> {
    ...
    for variant_state in state.enabled_variant_data().variant_states {
        ...
        let (data_pattern, return_pattern, return_type) = match &variant.fields {
            Fields::Named(_) => return Err(syn::parse::Error::new_spanned(&variant.ident, "AsVariant cannot be derived on enums with variants that have named fields")),
            Fields::Unnamed(fields) => {
                ...
                if fields.unnamed.len() == 1 {
                    ...
                    (data_pattern.clone(), data_pattern, quote! { &#ty })
                } else {
                    let field_tys = fields.unnamed.iter().map(|field| &field.ty);
                    (data_pattern.clone(), data_pattern, quote! { (#(&#field_tys),*) })
                }
            },
